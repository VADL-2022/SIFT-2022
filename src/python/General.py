...
def getUnfishmap():
    ...
    return unfishmap1, unfishmap2 # Usage: img_undistorted = cv2.remap(img, unfishmap1, unfishmap2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)

def unwarpNew(img):
    unfishmap1, unfishmap2 = getUnfishmap()
    return cv2.remap(img, unfishmap1, unfishmap2, interpolation=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)

def undistortPoints(point_matrix):
    getUnfishmap()
    return cv2.fisheye.undistortPoints(point_matrix,K,D,P=Knew)
...
def undistortImg(img, balance=0.0, dim2=None, dim3=None):
    ...
    if dim1[0]/dim1[1] != DIM[0]/DIM[1]:
        ...
        # print("undistortImg: cv2.resize took", timeit.timeit(fn_thing, number=1), "seconds")
        pass
    if not dim2:
        dim2 = dim1
    if not dim3:
    ...
    if map1_ is None or map2_ is None:
